...
export async function check1SubAuth(): Promise<boolean> {
  try {
    ...
    const response = await fetch('https://1sub.io/api/user/me', {
      method: 'GET',
...
export async function createCheckout(toolId: string): Promise<{ checkout_id: string; checkout_url: string }> {
  try {
    const response = await fetch('https://1sub.io/api/checkout/create', {
      method: 'POST',
    ...
    if (response.ok) {
      ...
      const checkout_url = `https://1sub.io/credit_checkout/${checkout_id}`;
      return {
        checkout_id,
        checkout_url,
      };
