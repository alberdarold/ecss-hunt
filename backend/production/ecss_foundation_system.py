...
class ECSSFoundationSystem:
    ...
    def _create_enhanced_search_result_with_content(self, chunk, query: str, detailed_content: str, index: int) -> Optional[SearchResult]:
        ...
        try:
            ...
            # Use the detailed content extraction (IMPROVED for engineer-friendly results!)
            if detailed_content and len(detailed_content) > 100:
                # Extract focused, relevant snippets based on query terms
                focused_content = self._extract_focused_content(detailed_content, query, index)
                content_text = focused_content['content']
                summary_text = focused_content['summary']
                
                # Create engineer-friendly title
                title = self._create_result_title(content_text, filename, query, is_visual)
    ...
    def _extract_focused_content(self, detailed_content: str, query: str, index: int) -> Dict[str, str]:
        """Extract focused, engineer-friendly content snippets."""
        try:
            query_terms = query.lower().split()
            sentences = detailed_content.split('. ')
            ...
            if relevant_sentences:
                # Take 2-3 most relevant sentences for this index
                start_idx = min(index * 2, len(relevant_sentences) - 1)
                end_idx = min(start_idx + 3, len(relevant_sentences))
                selected_sentences = relevant_sentences[start_idx:end_idx]
                content = '. '.join(selected_sentences) + '.'
                
                # Create summary from first sentence
                summary = selected_sentences[0][:150] + "..." if len(selected_sentences[0]) > 150 else selected_sentences[0]
            else:
                # Fallback: use paragraph-based extraction
                paragraphs = detailed_content.split('\n\n')
                if index < len(paragraphs):
                    content = paragraphs[index].strip()
                else:
                    # Use portion of content
                    start_pos = min((index * len(detailed_content)) // 5, len(detailed_content) - 400)
                    end_pos = min(start_pos + 400, len(detailed_content))
                    content = detailed_content[start_pos:end_pos].strip()
                
                # Create summary from first 150 characters
                summary = content[:150] + "..." if len(content) > 150 else content
            
            # Clean up content (max 500 characters for readability)
            if len(content) > 500:
                content = content[:500] + "..."
            ...
            logger.error(f"Error extracting focused content: {e}")
            return {
                'content': "Content extraction failed",
                'summary': "Unable to extract content"
            }
    
    def _create_result_title(self, content: str, filename: str, query: str, is_visual: bool) -> str:
        """Create engineer-friendly titles for search results."""
        try:
            # Extract document info from filename
            if 'ECSS-' in filename:
                # Parse ECSS filename: ECSS-[Branch]-[Discipline]-[Number]
                ...
                if len(parts) >= 4:
                    ...
                    branch_names = {'E': 'Engineering', 'M': 'Management', 'Q': 'Quality', 'S': 'Space Assurance', 'U': 'Sustainability'}
                    ...
                    elif 'definition' in content.lower() or 'means' in content.lower():
                        content_type = "Definition"
                    elif 'procedure' in content.lower() or 'method' in content.lower():
                        content_type = "Procedure"
                    elif 'table' in content.lower() or 'specification' in content.lower():
                    ...
                    # Create focused title based on query
                    query_focus = query.title() if len(query) < 20 else "Search Topic"
                    
                    return f"📋 {content_type}: {query_focus} | ECSS-{branch}-{discipline}-{number} ({branch_name})"
            
            # Fallback title
            content_preview = content[:50].replace('\n', ' ') + "..." if len(content) > 50 else content.replace('\n', ' ')
            return f"📄 {query.title()} | {content_preview}"
            
        except Exception as e:
            logger.error(f"Error creating result title: {e}")
